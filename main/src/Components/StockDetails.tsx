...
const StockDetails = () => {
    ...
    const getText = (criteria:any) => {
        let textData = criteria.text;
        Object.keys(criteria.variable).forEach((keyName, ind) => {
            if (Array.isArray(criteria.variable[keyName]?.values)) {
                if (textData.includes(keyName)) {
                    const tag = <span onClick = {() => navigate("/new")} className="text-indigo-600">${(criteria.variable[keyName]?.values as number[])[0]}</span>
                    console.log(tag)
                    textData = textData.replaceAll(keyName, tag);
